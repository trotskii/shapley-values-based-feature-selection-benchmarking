...
df = pd.read_csv('data/ionosphere/ionosphere.csv', sep=',', header=None)
...
# with open(f'results/ionosphere/results_ppfs.json', 'w') as file:
...
for n_words in n_words_options:
    for name, method in method_list.items():
        ...
        with open(f'results/ionosphere/results_{name}_{n_words}.json', 'w') as file:
            json.dump(result, file) 


result = get_baseline(df, model)
with open(f'results/ionosphere_baseline.json', 'w') as file:
    json.dump(result, file)