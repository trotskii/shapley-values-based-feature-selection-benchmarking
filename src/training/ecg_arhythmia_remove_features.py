...
def test_extractor(model: sklearn.base.BaseEstimator, extractor: filter.BaseTextFeatureExtractor, df: pd.DataFrame, n_features: int) -> dict:
    ...
    for train_idx, test_idx in group_k_fold.split(X, y, f_idx):
        ...
        X_train_filtered, features_filtered = extractor.remove_n_best(X_train, n_features, features)
        X_test_filtered, _ = extractor.remove_n_best(X_test, n_features, features)
        end = timer()
...
def shap_based_method(df: pd.DataFrame, model: sklearn.base.BaseEstimator, n_features: int) -> dict:
    ...
    for train_idx, test_idx in group_k_fold.split(X, y, f_idx):
        ...
        start = timer()
        X_train_filtered, filtered_features = extractor.remove_n_best(X_train, n_features)
        X_test_filtered, _ = extractor.remove_n_best(X_test, n_features)
        end = timer()
        timing['filtered_features'].append(timedelta(seconds=end-start))
...
def lfs_based_method(df: pd.DataFrame, model: sklearn.base.BaseEstimator, n_words: int) -> dict:
    ...
    for train_idx, test_idx in group_k_fold.split(X, y, f_idx):
        ...
        remove_cols = X_train.columns[extractor.selected_idx]
        X_train_filtered = X_train.drop(columns=remove_cols)        
        X_test_filtered = X_test.drop(columns=remove_cols) 
...
def main():
    ...
    # model = SVC(class_weight='balanced', gamma='scale', C=0.1)
    ...
    for n_words in n_words_options:
        for name, method in method_list.items():
            ...
            with open(f'results/ecg_mit_remove/results_{name}_{n_words}.json', 'w') as file:
